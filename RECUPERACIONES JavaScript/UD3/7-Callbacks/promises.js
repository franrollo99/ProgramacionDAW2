...
formulario.addEventListener('submit', (e) => {
    ...
    validarNombre(nombre.value).then(valor1 => {
        nombre.classList.remove('error');

        validarContraseña(contraseña.value).then(valor2 => {
            contraseña.classList.remove('error');

            validarEmail(email.value).then(valor3 => {
                email.classList.remove('error');

                validarFechaNacimiento(fechaNacimiento.value).then(valor4 => {
                ...
                }).catch(error4 => mostrarError(error4))
            }).catch(error3 => mostrarError(error3));
        }).catch(error2 => mostrarError(error2));
    }).catch(error1 => mostrarError(error1));
});
